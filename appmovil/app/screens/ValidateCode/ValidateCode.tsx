...
import { ValidateCodeProps } from "./types";
import {
  DEFAULT_INVALID_CODE_ERROR,
  EMPTY_CODE_ERROR,
...
const ValidateCode: React.FC<ValidateCodeProps> = ({ navigation, route, setIsAuth }) => {
  ...
  const handleValidate = async () => {
    ...
    try {
      const response = await fetch(VALIDATE_CODE_ROUTE, {
        method: "POST",
      ...
      if (!response.ok) {
        setError(data.message || DEFAULT_INVALID_CODE_ERROR);
        return;
      }
