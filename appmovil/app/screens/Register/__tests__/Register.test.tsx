...
describe("RegisterScreen", () => {
  ...
  it("renderiza correctamente todos los elementos", () => {
    ...
    expect(getByText(/Acepto la política de privacidad/i)).toBeTruthy();
    expect(getAllByText("Crear Cuenta").length).toBeGreaterThan(0);
  ...
  it("muestra error si no acepta la política de privacidad", () => {
  ...
  it("registra correctamente y navega a Login si fetch es exitoso", async () => {
    ...
    expect(mockNavigation.navigate).toHaveBeenCalledWith("Login", { message: "REGISTER_SUCCESS" });
  });
  ...
  it("coincide con el snapshot", () => {
    const { toJSON } = render(<RegisterScreen navigation={mockNavigation} route={mockRoute} />);
