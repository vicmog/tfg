    ...
    useEffect(() => {
        ...
        const timeout = setTimeout(async () => {
            ...
            try {
                const token = await AsyncStorage.getItem("token");
                const response = await fetch(
                    `http://localhost:3000/v1/api/users?search=${encodeURIComponent(trimmed)}`,
                    {
                        headers: {
                            Authorization: `Bearer ${token}`,
                        },
                ...
                if (response.ok) {
                    ...
                    const data = await response.json();
                    setModalError(data.message || "No se pudieron buscar usuarios");
                    setSearchResults([]);
                }
            } catch (err) {
    ...
    const handleGrantAccess = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const data = await response.json();
                setModalError(data.message || "No se pudo asignar el acceso");
            }
        } catch (err) {
            setModalError("Error de conexión");
        }
