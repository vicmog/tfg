...
const formatSlotLabel = (date: Date) => `${`${date.getHours()}`.padStart(2, "0")}:${`${date.getMinutes()}`.padStart(2, "0")}`;
...
const toIsoTimeKey = (value: string | Date) => {
    const date = new Date(value);
    return `${`${date.getHours()}`.padStart(2, "0")}:${`${date.getMinutes()}`.padStart(2, "0")}`;
};

const getInitialDuration = (reserva: Reserva) => {
    if (reserva.duracion_minutos && reserva.duracion_minutos > 0) {
        return `${reserva.duracion_minutos}`;
    }
...
const EditarReserva: React.FC<EditarReservaProps> = ({ route, navigation }) => {
    const { negocio, reserva } = route.params;
    ...
    const [datePickerCursor, setDatePickerCursor] = useState<Date>(new Date(reserva.fecha_hora_inicio));
    ...
    const filteredRecursos = useMemo(() => {
        ...
        return recursos.filter((item) => normalizeText(item.nombre).includes(query));
    }, [recursos, recursoSearchQuery]);

    const filteredServicios = useMemo(() => {
        const query = normalizeText(servicioSearchQuery.trim());
        if (!query) return servicios;
        return servicios.filter((item) => normalizeText(item.nombre).includes(query));
    }, [servicios, servicioSearchQuery]);

    const selectedDurationMinutes = useMemo(() => {
        if (!INTEGER_REGEX.test(duracionMinutos.trim())) return 0;
    ...
    const availableSlots = useMemo(() => {
        ...
        const resourceReservations = reservas.filter(
            (item) => item.id_reserva !== reserva.id_reserva
                && item.id_recurso === selectedRecursoId
                && toLocalDateKey(item.fecha_hora_inicio) === selectedFecha
        );

        const reservationRanges = resourceReservations
            .map((item) => ({
                startMs: new Date(item.fecha_hora_inicio).getTime(),
                endMs: new Date(item.fecha_hora_fin).getTime(),
            }))
        ...
        for (let minuteOfDay = openingMinutes; minuteOfDay + selectedDurationMinutes <= closingMinutes; minuteOfDay += slotStepMinutes) {
        ...
        return slots;
    }, [duracionMinutos, reserva.id_reserva, reservas, selectedDurationMinutes, selectedFecha, selectedRecursoId]);
            ...
            const fetchData = async () => {
                ...
                try {
                    ...
                    const [clientesResponse, recursosResponse, serviciosResponse, reservasResponse] = await Promise.all([
                        fetch(clientesByNegocioRoute(negocio.id_negocio), { headers: { Authorization: `Bearer ${token}` } }),
                        fetch(recursosByNegocioRoute(negocio.id_negocio), { headers: { Authorization: `Bearer ${token}` } }),
                        fetch(serviciosByNegocioRoute(negocio.id_negocio), { headers: { Authorization: `Bearer ${token}` } }),
                        fetch(reservasByNegocioRoute(negocio.id_negocio), { headers: { Authorization: `Bearer ${token}` } }),
                    ]);
                    ...
                    const fetchedServicios = serviciosData.servicios || [];
    ...
    const handleSave = async () => {
        ...
        try {
            const token = await AsyncStorage.getItem("token");
            const response = await fetch(updateReservaByIdRoute(reserva.id_reserva), {
                method: "PUT",
                headers: {
    ...
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                <TouchableOpacity style={styles.iconButton} onPress={() => navigation.goBack()} testID="editar-reserva-back-button">
            ...
            {error ? (
                <View style={styles.feedbackError} testID="editar-reserva-error-message">
            ...
            {loadingData ? (
                <View style={styles.loadingContainer}>
                    <ActivityIndicator size="large" color="#1976D2" testID="editar-reserva-loading-data" />
                </View>
            ) : (
                <ScrollView contentContainerStyle={styles.content}>
                    ...
                    <TouchableOpacity
                        ...
                        testID="editar-reserva-open-clientes-picker"
                    >
                    ...
                    <TouchableOpacity
                        ...
                        testID="editar-reserva-open-recursos-picker"
                    >
                    ...
                    <TouchableOpacity style={styles.selector} onPress={handleOpenDatePicker} testID="editar-reserva-date-picker-button">
                    ...
                    {datePickerVisible ? (
                        Platform.OS === "web" ? (
                            <View style={styles.inlineCalendarCard} testID="editar-reserva-date-picker">
                                <View style={styles.inlineCalendarHeader}>
                                    <TouchableOpacity onPress={() => setDatePickerCursor((prev) => new Date(prev.getFullYear(), prev.getMonth() - 1, 1))}>
                            ...
                            <DateTimePicker
                                testID="editar-reserva-native-date-picker"
                    ...
                    <TextInput
                        ...
                        keyboardType="number-pad"
                        testID="editar-reserva-duracion-input"
                    />

                    <Text style={styles.fieldLabel}>{FRANJA_LABEL}</Text>
                    <View style={styles.slotsContainer} testID="editar-reserva-slots-container">
                        {availableSlots.length === 0 ? (
                            <Text style={styles.emptyText}>{PICK_FRANJA_PLACEHOLDER}</Text>
                        ) : (
                            availableSlots.map((slot) => {
                                const isSelected = selectedSlotInicioIso === slot.iso
                                    || (selectedSlotInicioIso && toIsoTimeKey(selectedSlotInicioIso) === slot.label);
                                ...
                                return (
                                    <TouchableOpacity
                                        ...
                                        testID={`editar-reserva-slot-${slot.id}`}
                                    >
                    ...
                    <TouchableOpacity
                        ...
                        onPress={handleSave}
                        disabled={saving}
                        testID="editar-reserva-save-button"
                    >
            ...
            <Modal visible={clientePickerVisible} transparent animationType="fade" onRequestClose={() => setClientePickerVisible(false)}>
                <View style={styles.modalBackdrop}>
                    <View style={styles.modalCard}>
                        ...
                        <TextInput style={styles.searchInput} placeholder="Buscar cliente" value={clienteSearchQuery} onChangeText={setClienteSearchQuery} />
                        <ScrollView>
                            {clientes.length === 0 ? (
            ...
            <Modal visible={recursoPickerVisible} transparent animationType="fade" onRequestClose={() => setRecursoPickerVisible(false)}>
                <View style={styles.modalBackdrop}>
                    <View style={styles.modalCard}>
                        <View style={styles.modalHeader}>
                        ...
                        <TextInput style={styles.searchInput} placeholder="Buscar recurso" value={recursoSearchQuery} onChangeText={setRecursoSearchQuery} />
                        <ScrollView>
                            {recursos.length === 0 ? (
            ...
            <Modal visible={servicioPickerVisible} transparent animationType="fade" onRequestClose={() => setServicioPickerVisible(false)}>
                <View style={styles.modalBackdrop}>
                    <View style={styles.modalCard}>
                        <View style={styles.modalHeader}>
